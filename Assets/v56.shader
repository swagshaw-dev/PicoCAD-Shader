Shader "Custom/v56"
{
    Properties
    {
        ...
        [Toggle] _UniformObjectDither ("Use Uniform Object Dither", Float) = 0
        _UniformObjectScale ("Uniform Grid Scale", Float) = 1.0
    ...
    SubShader
    {
        ...
        // ----------------------------------------------------------------------
        // Plaid pattern: screen-space, dynamic, or uniform UV-space grid.
        // ----------------------------------------------------------------------
        float CalculatePlaid(float4 positionCS, float3 positionWS, float2 uv)
        {
            // Uniform Object Dither: uses noperspective UVs with a fixed scale
            // to guarantee zero perspective warping and stable dither bands.
            if (_UniformObjectDither > 0.5)
            {
                float2 grid = uv * _UniformObjectScale;
                return fmod(floor(grid.x) + floor(grid.y), 2.0);
            }
        ...
        Pass
        {
            ...
            half4 fragBackFaces(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                if (_AlphaMode < 0.5)
                {
                    if (abs(alpha - _Cutoff) <= _AlphaTolerance) clip(alpha - plaid);
                    else clip(alpha - _Cutoff);
                }
                else if (_AlphaMode < 1.5)
                {
                    if (abs(alpha - _Cutoff) <= _AlphaTolerance) clip(alpha - plaid);
                    else { clip(alpha - _Cutoff); clip(alpha - plaid); }
                }
