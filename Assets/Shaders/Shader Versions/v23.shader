Shader "Custom/v23"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Point-clamp sampler for the index texture.
            // Unity creates implicit samplers from their name: "point" sets nearest-
            // neighbour filtering and "clamp" sets the wrap mode. This ensures each
            // pixel reads exactly one texel with zero interpolation, which prevents
            // bilinear blending from mixing in adjacent texels across UV seams.
            // Without this, pixels near a seam can sample a blend of the intended
            // color and a neighboring white region, causing the palette loop to snap
            // to the wrong color and produce white speckles along mesh edges.
            ...
            {
                // --- Sample index texture with point filtering at mip 0.
                // Point sampling reads the single nearest texel — no blending between
                // neighbors. Combined with LOD 0, this means UV seams cannot bleed
                // adjacent texel colors into the palette lookup regardless of zoom level.
