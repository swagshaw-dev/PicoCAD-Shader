Shader "Custom/v32"
{
    Properties
    {
        ...
        [Enum(Screen Space Unmoving,0,Texture Space Moving,1)] _PlaidSpace("Plaid Coordinate Space", Float) = 0
        [Toggle] _DynamicPlaid("Use Dynamic Camera Scaling (Screen Space Only)", Float) = 0
        _DitherScaleStatic("Plaid Scale", Float) = 0.02
    ...
    SubShader
    {
        ...
        float CalculatePlaid(float4 positionCS, float3 positionWS, float2 uv) {
            if (_PlaidSpace > 0.5) {
                // Texture Space (Moving)
                float2 texCell = floor(uv * _IndexTex_TexelSize.zw * _DitherScaleStatic * 10.0);
                return fmod(texCell.x + texCell.y, 2.0);
            } else {
                // Screen Space (Unmoving)
        ...
        Pass
        {
            ...
            half4 fragBackFaces(Varyings IN) : SV_Target {
                ...
                float plaid = CalculatePlaid(IN.positionCS, IN.positionWS, IN.uv);

                if (_AlphaMode < 0.5) clip(inputSample.a - _Cutoff);
                else if (_AlphaMode < 1.5) { clip(inputSample.a - 1.0/255.0); clip(inputSample.a - plaid); }
                ...
                if (_BackFaceMode < 0.5) return half4(SamplePalette(colorIdx, 0), (_AlphaMode > 1.5) ? inputSample.a : 1.0);
                if (_BackFaceMode > 0.5 && _BackFaceMode < 1.5) return half4(SamplePalette(colorIdx, 3), (_AlphaMode > 1.5) ? inputSample.a : 1.0);

                // Full lighting logic for BackFaces...
                ...
                float lightFactor = saturate(dot(normalWS, mainLight.direction)) * step(_ShadowCutoff, mainLight.shadowAttenuation);
                
                float shaped = floor(clamp(lightFactor * _LightScale, _Ambient, 1.0) * _LightSteps + 0.5) / _LightSteps;
                bool dithBit = (plaid < 0.5);
                int row = (shaped < _ThreshBlack) ? 3 : (shaped < _ThreshDark) ? (dithBit ? 3 : 2) : (shaped < _ThreshMid) ? (dithBit ? 2 : 1) : (shaped < _ThreshDither) ? (dithBit ? 1 : 0) : 0;
                return half4(SamplePalette(colorIdx, row), (_AlphaMode > 1.5) ? inputSample.a : 1.0);
            }
            ENDHLSL
        }
