Shader "Custom/v26"
{
    Properties
    {
        ...
        _PaletteTex("Add Palette Here (16 x 4)", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float  _Cutoff;
                float  _Cull;
                // 16 columns x 4 rows baked from _PaletteTex via the inspector "Bake" button.
                // Indexed as _Palette[row * 16 + col]. Pure register reads — no texture fetches.
                float4 _Palette[64];
            CBUFFER_END
            ...
            /// Returns the baked palette color for the given column and row.
            float3 SamplePalette(int idx, int row)
            {
                return _Palette[row * 16 + idx].rgb;
            }
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // _Palette is a CBUFFER register array — no texture fetches, no serialized
                // latency. The compiler fully unrolls this loop across 16 known-count iterations.
                int   colorIdx = 0;
                float minDist  = 1e9;
                for (int p = 0; p < 16; p++)
                {
