Shader "Custom/v15"
{
    Properties
    {
        ...
        _ThreshBand("Dither Band (min dark-mid gap)", Range(0, 0.5)) = 0.05
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                //
                // Thresholds are enforced to be in ascending order.
                // _ThreshBand sets the minimum gap between tDark and tMid, guaranteeing
                // the dark↔mid dither zone always has non-zero width. This prevents the
                // sharp two-row contrast jump (row 2 → row 0) at geometry silhouette edges
                // that occurs when _ThreshMid is at or below _ThreshDark.
                //
                // Both transitions now dither between ADJACENT rows only:
                //   [tDark, tMid]    → dither row 2 ↔ row 1
                //   [tMid, tDither]  → dither row 1 ↔ row 0
                //
                float tDark   = _ThreshDark;
                float tMid    = max(_ThreshMid, tDark + _ThreshBand);   // enforce minimum band
        ...
        // ------------------------------------------------------------------ //
        // ShadowCaster — custom: clips _IndexTex alpha so transparent regions
        // don't cast shadows.
        // ------------------------------------------------------------------ //
        Pass
        {
