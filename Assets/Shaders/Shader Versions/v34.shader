Shader "Custom/v34"
{
    ...
    SubShader
    {
        ...
        // Plaid scale is either static or camera-distance-driven per toggle.
        //
        ...
        Pass
        {
            ...
            half4 fragBackFaces(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // All three modes hard-clip below _Cutoff first.
                // Stipple then additionally clips the checkerboard pattern.
                // Blend passes the remaining alpha through to the blender.
                if (_AlphaMode < 0.5)
                {
                    // None: hard cutoff only.
                    clip(alpha - _Cutoff);
                }
                else if (_AlphaMode < 1.5)
                {
                    // Stipple: hard cutoff, then checkerboard the remainder.
                    clip(alpha - _Cutoff);
                    clip(alpha - plaid);
                }
                else
                {
                    // Blend: hard cutoff, then blend the remainder.
                    clip(alpha - _Cutoff);
                }
