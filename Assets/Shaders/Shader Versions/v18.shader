    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // Flip normal on back faces so lighting is correct when double-sided
                float3 normalWS = normalize(IN.normalWS) * (facing > 0.0 ? 1.0 : -1.0);
                ...
                //
                // At geometry silhouette edges, adjacent pixels belong to faces with
                // very different normals, so `shaped` jumps sharply across one pixel.
                // ddx/ddy measure this per-pixel rate of change in screen space. Using
                // it as an adaptive dither band ensures threshold crossings are spread
                // across a dither region (mixing adjacent rows) rather than landing on
                // a single hard pixel boundary — eliminating the jagged seam regardless
                // of cube angle or orientation.
                //
                // Because `shaped` is already quantized, ddx/ddy are zero inside any
                // step — the band only opens when adjacent pixels are in different steps,
                // which is precisely the geometry edge or lighting transition we want
                // to dither. _ThreshBand is a user-tunable minimum that applies
                // everywhere; the gradient raises it automatically only at those points.
                //
                // Thresholds are enforced to be in ascending order. Both transitions
                // dither between ADJACENT rows only:
                ...
                int paletteRow;
                if (shaped < tDark)
                    paletteRow = 2;                 // fully dark
                else if (shaped < tMid)
                    paletteRow = dithBit ? 2 : 1;  // dither dark ↔ mid
                else if (shaped < tDither)
                    paletteRow = dithBit ? 1 : 0;  // dither mid ↔ bright
                else
                    paletteRow = 0;                 // fully bright
