Shader "Custom/v63"
{
    ...
    SubShader
    {
        ...
        // ----------------------------------------------------------------------
        // Geometry Shader: Robust Flat Normal Calculation
        // ----------------------------------------------------------------------
        [maxvertexcount(3)]
        void geom(triangle Varyings input[3], inout TriangleStream<Varyings> output)
        {
            float3 v0OS = input[0].positionOS;
            float3 v1OS = input[1].positionOS;
            float3 v2OS = input[2].positionOS;
            
            float3 flatNormalOS = normalize(cross(v1OS - v0OS, v2OS - v0OS));
        ...
        // ----------------------------------------------------------------------
        // Shared shading logic for both passes
        // ----------------------------------------------------------------------
        half4 ComputeShadedColor(Varyings IN, bool isBackFace)
        {
            ...
            // 🔪 HARD TERMINATOR GATE - Replaces quadratic curve with immediate zero-crossing snap
            float rawDot = dot(normalWS, mainLight.direction) * lightSign;
            ...
            float adaptiveBand = _ThreshBand;
