Shader "Custom/v24"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // --- Sample index texture with point-clamp at mip 0.
                ...
                // --- Normal resolution
                // Flat shading derives the geometric face normal from ddx/ddy of world
                // position. The GPU computes screen-space derivatives in 2x2 pixel quads.
                // At a triangle boundary, pixels in the same quad can belong to different
                // triangles, so ddx/ddy of positionWS spans two different planes and the
                // resulting cross product does not correspond to either face.
                //
                // Guard: compare the candidate geometric normal against the interpolated
                // vertex normal. If their dot product is below 0.5 in absolute value
                // (angle > 60°), the derivative crossed a triangle boundary and the
                // geometric normal is unreliable — fall back to the vertex normal, which
                // is always valid. This eliminates the speckled dots along mesh edges
                // caused by the bad normal feeding a wrong lambert value into the
                // palette threshold evaluation.
                ...
                if (_FlatShading > 0.5)
                {
                    ...
                    // else: keep vertexNormalWS — derivative straddled a triangle boundary
                }

                // Resolve shadow coordinate
                #if defined(REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR)
