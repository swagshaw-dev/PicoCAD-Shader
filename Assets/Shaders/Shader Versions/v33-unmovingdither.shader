Shader "Custom/v33"
{
    ...
    SubShader
    {
        ...
        float CalculatePlaid(float4 positionCS, float3 positionWS, float2 uv) {
            ...
            if (_PlaidSpace < 0.5) {
                // Screen Space — fixed to screen, does not move with object
                ...
                // World Space — snapped to a world grid, moves with the object
                float2 worldCell = floor(positionWS.xz / max(0.0001, finalScale));
        ...
        Pass
        {
            ...
            half4 fragBackFaces(Varyings IN, half facing : VFACE) : SV_Target {
                ...
                else if (_AlphaMode < 1.5) { clip(alpha - 1.0/255.0); clip(alpha - plaid); }
                ...
                // Lighting calculation logic restored from v31
                ...
                int paletteRow = (shaped < tBlack) ? 3 : (shaped < tDark) ? (dithBit ? 3 : 2) : (shaped < tMid) ? (dithBit ? 2 : 1) : (shaped < tDither) ? (dithBit ? 1 : 0) : 0;
