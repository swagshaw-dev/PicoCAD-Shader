Shader "Custom/v27"
{
    Properties
    {
        ...
        [Enum(None,0,Stipple,1,Blend,2)] _AlphaMode("Alpha Mode (None)", Float) = 2
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // --- Screen-space checker (threshold plaid) — computed early so it is
                // available for both the stipple alpha clip and the lighting dither below.
                ...
                // --- Alpha mode
                // None:    hard cutoff — discard pixels below _Cutoff.
                // Stipple: checker-based discard driven by texture alpha;
                //          denser alpha = more surviving pixels, same cell size as lighting dither.
                // Blend:   no discard — alpha passes through to the blended output.
                if (_AlphaMode < 0.5)
                    clip(alpha - _Cutoff);
                else if (_AlphaMode < 1.5)
                {
                    // Hard discard for fully-transparent pixels so zero-alpha areas
                    // (e.g. holes) clip cleanly without the checker keeping half of them.
                    clip(alpha - 1.0 / 255.0);
                    clip(alpha - plaid);
                }

                // Surviving pixels are fully opaque for None and Stipple; Blend passes alpha.
        ...
        Pass
        {
            ...
            {
                // Blend mode should not write depth — clip everything so the
                // depth prepass cannot cause back-face self-occlusion.
                clip(_AlphaMode > 1.5 ? -1.0 : 1.0);

                // None/Stipple: hard cutoff for stable depth shapes.
        ...
        Pass
        {
            ...
            {
                // Always hard cutoff for consistent shadow shapes across all alpha modes.
                float alpha = SAMPLE_TEXTURE2D(_IndexTex, sampler_IndexTex, IN.uv).a;
                clip(alpha - _Cutoff);
                return 0;
            }
