Shader "Custom/v55"
{
    Properties
    {
        ...
        [Toggle] _FlatPlaid ("Use Flat Plaid (World-space)", Float) = 0
        _FlatPlaidScale ("Flat Plaid Scale (World Units)", Float) = 1.0
        _AlphaTolerance ("Alpha Tolerance", Range(0,1)) = 0.0
    ...
    SubShader
    {
        ...
        // ----------------------------------------------------------------------
        // Plaid pattern: can be screen‑space (original) or flat world‑space.
        // ----------------------------------------------------------------------
        float CalculatePlaid(float4 positionCS, float3 positionWS)
        {
            // Flat (world‑space) mode – ignores the camera and uses world coordinates.
            if (_FlatPlaid > 0.5)
            {
                // Prevent division by zero.
                float scale = max(_FlatPlaidScale, 0.0001);
                float2 worldCell = floor(positionWS.xy / scale);
                return fmod(worldCell.x + worldCell.y, 2.0);
            }

            // Original screen‑space version (static or distance‑scaled).
            ...
            // Convert clip‑space XY to a cell index.
        ...
        Pass
        {
            ...
            {
                // ------------------------------------------------------------------
                // 1️⃣  Sample the indexed texture (the source image)
                // ------------------------------------------------------------------
                ...
                // ------------------------------------------------------------------
                // 2️⃣  Compute the plaid pattern (screen‑space or world‑space)
                // ------------------------------------------------------------------
                float plaid = CalculatePlaid(IN.positionCS, IN.positionWS);

                // ------------------------------------------------------------------
                // 3️⃣  Alpha handling – now includes a tolerance band.
                // ------------------------------------------------------------------
                if (_AlphaMode < 0.5)
                {
                    // Simple cutoff. If we are within the tolerance band we fall back
                    // to a plaid‑based dither, otherwise we just cut off.
                    if (abs(alpha - _Cutoff) <= _AlphaTolerance)
                    {
                        // Within tolerance → use plaid as a binary dither.
                        clip(alpha - plaid);
                    }
                    else
                    {
                ...
                {
                    // Original mode: alpha AND plaid cut.
                    // The tolerance band widens the region where the plaid can
                    // rescue a pixel that would otherwise be discarded.
                    if (abs(alpha - _Cutoff) <= _AlphaTolerance)
                    {
                        clip(alpha - plaid);
                    }
                ...
                // ------------------------------------------------------------------
                // 4️⃣  Determine which palette colour we should use.
                // ------------------------------------------------------------------
                int colorIdx = 0;
                float minDist = 1e9;
                for (int p = 0; p < 16; p++)
                ...
                // ------------------------------------------------------------------
                // 5️⃣  Choose the palette row based on lighting / dither.
                // ------------------------------------------------------------------
                // (lighting code unchanged – omitted for brevity)
                // We keep the original lighting block unchanged, only the
                // alpha handling above was modified.
                // ------------------------------------------------------------------

                // Back‑face specific shading
                if (isBackFace && _BackFaceMode < 0.5) return half4(SamplePalette(colorIdx, 0), (_AlphaMode > 1.5) ? alpha : 1.0);
                if (isBackFace && _BackFaceMode > 0.5 && _BackFaceMode < 1.5) return half4(SamplePalette(colorIdx, 3), (_AlphaMode > 1.5) ? alpha : 1.0);

                // ------------------------------------------------------------------
                // 6️⃣  Full lighting (same as the original shader)
                // ------------------------------------------------------------------
                ...
                return half4(SamplePalette(colorIdx, paletteRow), (_AlphaMode > 1.5) ? alpha : 1.0);
            }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // Alpha handling with tolerance (same logic as back‑face pass)
                if (_AlphaMode < 0.5)
                {
                    if (abs(alpha - _Cutoff) <= _AlphaTolerance)
                        clip(alpha - plaid);
                ...
                // Back‑face early‑out (same as original)
                if (isBackFace && _BackFaceMode < 0.5) return half4(SamplePalette(colorIdx, 0), (_AlphaMode > 1.5) ? alpha : 1.0);
                if (isBackFace && _BackFaceMode > 0.5 && _BackFaceMode < 1.5) return half4(SamplePalette(colorIdx, 3), (_AlphaMode > 1.5) ? alpha : 1.0);

                // ------------------------------------------------------------------
                // Lighting (identical to the original shader)
                // ------------------------------------------------------------------
        ...
        // ----------------------------------------------------------------------
        // DepthOnly Pass (unchanged, but we keep the original cutoff)
        // ----------------------------------------------------------------------
        Pass
        {
            Name "DepthOnly"
            ...
            {
                // Simple cutoff – we keep the original behaviour here.
