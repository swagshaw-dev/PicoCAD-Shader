    ...
    SubShader
    {
        ...
        float CalculatePlaid(float4 positionCS, float3 positionWS) {
            ...
            if (_DynamicPlaid > 0.5) {
                ...
                // Guard: if Near >= Far the range is degenerate — clamp so near=0, far=near+1
        ...
        // ------------------------------------------------------------------ //
        // Pass 0 — Depth Prime (back faces only, color-less)
        // Writes back-face depth so the back-face color pass always sits
        // behind the front-face color pass regardless of triangle order.
        // ------------------------------------------------------------------ //
        Pass
        {
            Name "DepthPrime"
            ...
            HLSLPROGRAM
            #pragma vertex vert
            #pragma fragment fragDepthPrime

            half4 fragDepthPrime(Varyings IN) : SV_Target {
                if (_Cull > 1.5) clip(-1.0);
                float2 snappedUV = (floor(IN.uv * _IndexTex_TexelSize.zw) + 0.5) * _IndexTex_TexelSize.xy;
                float alpha = SAMPLE_TEXTURE2D_LOD(_IndexTex, sampler_point_clamp, snappedUV, 0).a;
                clip(alpha - _Cutoff);
                // For stipple mode also clip against plaid so depth matches color
        ...
        // ------------------------------------------------------------------ //
        // Pass 1 — Back faces (color, ZWrite Off)
        // ------------------------------------------------------------------ //
        Pass
        {
            Name "BackFaces"
