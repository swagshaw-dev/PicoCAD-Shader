    ...
    SubShader
    {
        ...
        void geom(triangle Varyings input[3], inout TriangleStream<Varyings> output)
        {
            ...
            if (_FlatShading > 0.5)
            {
                ...
                // 3. Lighting Evaluation at Centroid — mirrors smooth-mode pipeline exactly
                float4 shadowCoord = TransformWorldToShadowCoord(centroidWS);
                Light mainLight = GetMainLight(shadowCoord);
                float lightSign = (_InvertLight > 0.5) ? -1.0 : 1.0;

                float lambert = saturate(dot(flatNormalWS, mainLight.direction) * lightSign);
                ...
                // Additional lights (point/spot) cannot be evaluated here — the clustered
                // light loop requires screen-space tile UV which does not exist in the
                // geometry stage. Only the main (directional) light is baked here.
                // The fragment stage adds additional lights on top via IN.positionWS.

                // 4. Store raw directional lightFactor — fragment will add additional lights
                //    then apply curve + quantize so both paths stay consistent.
                
                faceLightVal = lightFactor; // directional only — fragment adds point/spot lights
            }
        ...
        half4 ComputeShadedColor(Varyings IN, bool isBackFace)
        {
            ...
            // ✅ FIX: Apply cutoff unconditionally so it works in Blend mode too.
            // Discards fully transparent pixels to prevent overdraw & depth-sorting artifacts.
            clip(alpha - _Cutoff);

            if (_AlphaMode > 0.5 && _AlphaMode < 1.5)
            {
                // Stipple mode: apply dither/plaid mask on top of cutoff
            ...
            if (isBackFace) 
            {
                ...
                {
                    // Flat Light: always row 0
                    row = 0;
                }
                else if (_BackFaceMode < 1.5)
                {
                    // Flat Dark: always row 3
                    row = 3;
                }
                else
                {
                    // Light Scale: evaluate dynamic lights on back faces
                    // Follow Stepped Shading toggle to pick between flat or smooth normals
                    float3 normalWS;
                    if (_FlatShading > 0.5)
                    {
                        // Flat mode: use face normal from derivatives
                        normalWS = normalize(cross(ddy(IN.positionWS), ddx(IN.positionWS))) * -1.0;
                    }
                    else
                    {
                        // Smooth mode: use interpolated normal, flipped for backface
                        normalWS = normalize(IN.normalWS) * -1.0;
                    }
            ...
            else
            {
                if (_FlatShading > 0.5)
                {
                    // FLAT MODE: faceLightVal holds the main (directional) light only.
                    // Additional lights (point/spot) are added here in the fragment stage
                    // where the clustered light loop has valid screen-space tile data.
                    float lightFactor = IN.faceLightVal;

                    // Reconstruct the face normal from derivatives — matches the geom shader normal.
                    float3 faceNormalWS = normalize(cross(ddy(IN.positionWS), ddx(IN.positionWS)));
                    float lightSign = (_InvertLight > 0.5) ? -1.0 : 1.0;

                    // Required by _CLUSTER_LIGHT_LOOP macro expansion for tile/cluster lookup
                    InputData inputData = (InputData)0;
                    inputData.positionWS = IN.positionWS;
                    inputData.normalizedScreenSpaceUV = GetNormalizedScreenSpaceUV(IN.positionCS);
                    ...
                    LIGHT_LOOP_BEGIN((uint)GetAdditionalLightsCount())
                        Light addLight = GetAdditionalLight(lightIndex, IN.positionWS);
                        float addLambert = saturate(dot(faceNormalWS, addLight.direction) * lightSign);
                        float addShadow = step(_ShadowCutoff, addLight.shadowAttenuation);
                        lightFactor = saturate(lightFactor + addLambert * addShadow * addLight.distanceAttenuation);
                    LIGHT_LOOP_END

                    // Curve + quantize — identical to smooth mode
                    float shaped = 1.0 - (1.0 - lightFactor) * (1.0 - lightFactor);
                    shaped = clamp(shaped * _LightScale, _Ambient, 1.0);
                    shaped = floor(shaped * _LightSteps + 0.5) / _LightSteps;
                ...
                else
                {
                    ...
                    // Always compute shadow coord per-fragment in smooth mode.
                    // With a geometry shader in the pipeline, the vertex-interpolated
                    // shadowCoord (REQUIRES_VERTEX_SHADOW_COORD_INTERPOLATOR path) can
                    // select the wrong cascade at distance, snapping the light off.
                    #if defined(MAIN_LIGHT_CALCULATE_SHADOWS) || defined(_MAIN_LIGHT_SHADOWS) || defined(_MAIN_LIGHT_SHADOWS_CASCADE)
                        float4 shadowCoord = TransformWorldToShadowCoord(IN.positionWS);
                    #else
        ...
        Pass
        {
            ...
            half4 fragBackFaces(Varyings IN, half facing : VFACE) : SV_Target
            {
                ...
                // Skip rendering if in Front-only mode
        ...
        Pass
        {
            ...
            {
                bool isBackFace = (facing < 0.0);
                // _Cull: Front=2, Back=1, Both=0
                // Clip backfaces if mode is Front (2) only
                if (_Cull > 1.5 && isBackFace) clip(-1.0);
                return ComputeShadedColor(IN, isBackFace);
            }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            {
                // Blend mode must never write depth — stale depth entries cause
                // transparent geometry to pop in/out at cascade boundaries.
