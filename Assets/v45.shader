Shader "Custom/v45"
{
    Properties
    {
        ...
        _Ambient("Ambient (Light Floor)", Range(0,1)) = 0.15
        ...
        _ShadowCutoff("Shadow Sharpness", Range(0,1)) = 0.1
        _LightScale("Light Intensity Scale", Range(0, 4)) = 1.65
    ...
    SubShader
    {
        ...
        int GetShadedRow(float lightFactor, float plaid) {
            // v25 Shading Logic
            ...
            if (shaped < _ThreshBlack) return 3;
            if (shaped < max(_ThreshDark, _ThreshBlack + adaptiveBand)) return dithBit ? 3 : 2;
            if (shaped < max(_ThreshMid, _ThreshDark + adaptiveBand)) return dithBit ? 2 : 1;
            if (shaped < max(_ThreshDither, _ThreshMid + adaptiveBand)) return dithBit ? 1 : 0;
            return 0;
        }
        ENDHLSL
        ...
        Pass
        {
            ...
            half4 fragBackFaces(Varyings IN, half facing : VFACE) : SV_Target {
                ...
                if (_BackFaceMode < 1.5) return half4(SamplePalette(colorIdx, (_BackFaceMode < 0.5) ? 0 : 3), 1.0);
                ...
                float lightFactor = saturate(dot(normalWS, mainLight.direction) * lightSign) * mainLight.shadowAttenuation;

                for (int i = 0; i < GetAdditionalLightsCount(); i++) {
                    Light addLight = GetAdditionalLight(i, IN.positionWS);
                    lightFactor += saturate(dot(normalWS, addLight.direction) * lightSign) * addLight.distanceAttenuation * addLight.shadowAttenuation;
                }

                return half4(SamplePalette(colorIdx, GetShadedRow(saturate(lightFactor), plaid)), 1.0);
            }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN, half facing : VFACE) : SV_Target {
                ...
                return half4(SamplePalette(colorIdx, GetShadedRow(saturate(lightFactor), plaid)), (_AlphaMode > 1.5) ? inputSample.a : 1.0);
            }
            ENDHLSL
        }
    }
